...
Kele = np.zeros((neles, 2*ndpn, 2*ndpn))  
for i in range(neles): 
    ...
    Kele[i] = compute_K_matrix(ele[i][3] * ele[i][4] , ele[i][3] * ele[i][5], LO[i], dircos[i]) # EA, EI, L, phi
    print(Kele[i])
...
for i in range(neles):  
    ...
    for row in range(2 * ndpn):
        if glist[row] < ndofs:  # Only for unknown DOFs
            for col in range(2 * ndpn):
...
for i in range(neles):
    node1, node2 = int(ele[i][1]), int(ele[i][2])
