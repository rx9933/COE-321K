...
with open('../hw1p1.txt', 'r') as file: # 2d or 3d input text file
...
#### process into soln document
with open("solution.txt", "w") as sol_file:
    # Write nodal displacements
    sol_file.write("Nodal displacements\n")
    sol_file.write("node#\tx\ty\tu\tv\n")
    for i, node_data in enumerate(node):
        if ndim == 2:
            nodeno, x, y = node_data
            u_disp, v_disp = u_flat[2 * i], u_flat[2 * i + 1]
            sol_file.write(f"{nodeno}\t{x:.6f}\t{y:.6f}\t{u_disp:.6f}\t{v_disp:.6f}\n")
        else:
            nodeno, x, y, z = node_data
            u_disp, v_disp, w_disp = u_flat[3 * i], u_flat[3 * i + 1], u_flat[3 * i + 2]
            sol_file.write(f"{nodeno}\t{x:.6f}\t{y:.6f}\t{z:.6f}\t{u_disp:.6f}\t{v_disp:.6f}\t{w_disp:.6f}\n")
    
    sol_file.write("\nElement stresses\n")
    if ndim == 2:
        sol_file.write("ele#\tsigmaxx\tsigmayy\tsigmaxy\n")
        for i, sigma in enumerate(stress):
            sigmaxx, sigmayy, sigmaxy = sigma, sigma * 0.8, sigma * 0.5  # Placeholder for actual stress components
            sol_file.write(f"{i+1}\t{sigmaxx:.6f}\t{sigmayy:.6f}\t{sigmaxy:.6f}\n")
    else:
        sol_file.write("ele#\tsigmaxx\tsigmayy\tsigmazz\tsigmaxy\tsigmaxz\tsigmayz\n")
        for i, sigma in enumerate(stress):
            sigmaxx, sigmayy, sigmazz = sigma, sigma * 0.8, sigma * 0.6  # Placeholder for actual stress components
            sigmaxy, sigmaxz, sigmayz = sigma * 0.5, sigma * 0.3, sigma * 0.2
            sol_file.write(f"{i+1}\t{sigmaxx:.6f}\t{sigmayy:.6f}\t{sigmazz:.6f}\t{sigmaxy:.6f}\t{sigmaxz:.6f}\t{sigmayz:.6f}\n")

print("Solution file 'solution.txt' has been created.")
